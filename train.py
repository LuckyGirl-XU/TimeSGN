...
parser.add_argument('--eval_can_samples', type=int, default=1, help='how many candidate samples to use at inference.')
parser.add_argument('--DTMP', action='store_true')
...
from utils.compute_recall_at_k import *
...
sys.stdout = Logger(osp.join('./log/', args.model_name + '.txt'))
...
seperate_first = args.DTMP
model = GeneralModel(gnn_dim_node, gnn_dim_edge, sample_param, memory_param, gnn_param, train_param, seperate = seperate_first).cuda()
...
if args.use_inductive:
    ...
    print("inductive nodes", len(inductive_nodes), ', Inductive edges: ', len(test_df))
...
def eval(mode='val'):
    ...
    if mode == 'val':
        ...
        neg_samples = args.eval_can_samples
    elif mode == 'train':
    ...
    with torch.no_grad():
        ...
        for _, rows in eval_df.groupby(eval_df.index // train_param['batch_size']):
            ...
            if neg_samples > 1:
                ...
                hit_k.append(compute_recall_at_k(pred_pos, pred_neg, 1)) #recall@1
            else:
                aucs_mrrs.append(roc_auc_score(y_true, y_pred))
                hit_k.append(torch.tensor([0]))
            if mailbox is not None:
    ...
    if neg_samples > 1:
        ...
        hit_k = float(torch.tensor(hit_k).mean())
    else:
        auc_mrr = float(torch.tensor(aucs_mrrs).mean())
        hit_k = 0.0
    return ap, auc_mrr, hit_k
...
if args.eval_can_samples > 1:
    t_test = time.time()
    ap, auc, hitk = eval('test')
    time_test = time.time() - t_test
    print('\ttest AP:{:4f}  test MRR:{:4f}  test recall@1:{:4f}  test time:{:4f}'.format(ap, auc, hitk, time_test))
    
else:
    t_test = time.time()
    ap, auc, hitk = eval('test')
    time_test = time.time() - t_test
    print('\ttest AP:{:4f}  test AUC:{:4f}  test recall@1K:{:4f}   test time:{:4f}'.format(ap, auc, hitk, time_test))
