    ...
    if node_feats is not None:
        for b in mfgs[0]:
            if pinned:
                ...
                b.srcdata['h'] = nfeat_buffs[i][:idx.shape[0]].cuda(non_blocking=True) #node feature
                i += 1
            else:
                srch = node_feats[b.srcdata['ID'].long()].float()
                b.srcdata['h'] = srch.cuda() #node feature
    i = 0
            ...
            for b in mfg:
                if b.num_src_nodes() > b.num_dst_nodes():
                    if pinned:
                        ...
                        efeat_buffs[i] = torch.index_select(edge_feats, 0, idx)
                        ...
                        b.edata['f'] = srch.cuda() #edge features
    return mfgs
